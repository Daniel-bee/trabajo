...
def home():
    ...
    now = datetime.now()
    return render_template('home.html', title="Home", jobposts=jobposts, today=now.date() )
...
def searchJobs():
    ...
    if not keyword:
        page = request.args.get('page', 1, type=int)
        jobposts = JobPost.query.order_by(JobPost.created_at.desc()).paginate(page=page, per_page=7)
        return render_template('home.html',jobposts=jobposts, form=form, today=now.date())
...
def checkApplicantStatus(jobid, candidate_id):
    applicant = JobApply.query.filter_by(job_id=jobid, user_id=candidate_id).first()
    if applicant:
...
def jobdetail(postid):
    ...
    return render_template('jobs/jobdetails.html',
                        jobpost=jobpost, job_applicant=job_applicant, 
                        application=checkApplication, checkApplicantStatus=checkApplicantStatus )
...
def employerProfileDetail(userid):
    ...
    employer = {
        ...
        'today': todayWithoutTime,
        'count_applicant': countApplicant,
        'detail': EmployerInfo.query.filter_by(employer_id=userid).first()
    }
...
def employerInfo(empid):
    ...
    if request.method == 'POST' and employer in [None]:
        if form.validate_on_submit():
            create = EmployerInfo(
                about = form.about.data,
            ...
            db.session.commit()
            flash("Saved!", 'success')
            return redirect(url_for('employerInfo', empid=empid))
